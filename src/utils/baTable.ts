...
export default class baTable {
    ...
    // 查看
    getList = () => {
        if (!this.runBefore('getList')) return
        this.table.loading = true
        return this.api
            .list(this.table.filter)
            .then((res) => {
                this.table.data = res.data.list
                this.table.total = res.data.paginator ? res.data.paginator.total : 0
                    ...
                    if (data.field.render == 'switch') {
                        ...
                        this.api
                            .postData('status', {
                                ...
                                [data.field.prop]: parseInt(data.value)
                            })
                            .then(() => {
    ...
    afterSort = () => {
        if (this.table.defaultOrder && this.table.defaultOrder.prop) {
            ...
            if (this.table.filter && this.table.filter.order != undefined) {
                const sp = this.table.filter.order.split(',');
                this.table.ref.getRef()?.sort(sp[0], sp[1] == 'desc' ? 'descending' : 'ascending')
            }
        }
    }
    /**
